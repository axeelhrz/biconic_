...
async function queryFirebird(opts, sql) {
...
async function materializeFirebirdTable(conn, table, columns, dateFilter, pgUrl, targetSchema, targetTable, signal, sharedPgClient) {
    ...
    try {
        ...
        while (true) {
            ...
            offset += rows.length;
            if (rows.length < effectiveFbBatch)
                break;
        }
