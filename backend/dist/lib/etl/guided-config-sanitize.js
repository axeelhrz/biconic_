...
exports.sanitizeGuidedJoinForRun = sanitizeGuidedJoinForRun;
exports.resolvePrimaryConnectionId = resolvePrimaryConnectionId;
function readTableFromFilter(filter) {
    if (!filter || typeof filter !== "object")
        return "";
    return String(filter.table ?? "").trim();
}
function sanitizeGuidedJoinForRun(join, filter) {
    if (!join || typeof join !== "object")
        return null;
    const joinObj = join;
    const filterTable = readTableFromFilter(filter);
    const isStar = joinObj.primaryConnectionId != null &&
        String(joinObj.primaryConnectionId).trim() !== "" &&
        Array.isArray(joinObj.joins);
    if (isStar) {
        const validJoins = joinObj.joins.filter((jn) => !!jn &&
            typeof jn === "object" &&
        ...
        if (validJoins.length === 0) {
            return {
                ok: false,
                error: "JOIN estrella inválido: faltan tablas secundarias o conexiones. Editá el ETL y volvé a guardarlo.",
            };
        }
        ...
        if (!primaryTable) {
            return {
                ok: false,
                error: "JOIN estrella inválido: falta tabla principal (primaryTable). Editá el ETL y volvé a guardarlo.",
            };
        }
        return {
            ok: true,
    ...
    if (!secondaryConnectionId || !leftTable || !rightTable) {
        return {
            ok: false,
            error: "JOIN simple inválido: faltan conexión secundaria o tablas. Editá el ETL y volvé a guardarlo.",
        };
    }
    return { ok: true, join: joinObj };
}
function resolvePrimaryConnectionId(guided, join) {
...
//# sourceMappingURL=guided-config-sanitize.js.map