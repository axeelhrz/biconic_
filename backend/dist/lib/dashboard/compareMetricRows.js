...
exports.resolveRowColumnKey = resolveRowColumnKey;
exports.getRowValue = getRowValue;
exports.shiftBucketLabelOneYear = shiftBucketLabelOneYear;
exports.shiftCalendarBucketLabel = shiftCalendarBucketLabel;
exports.compareBucketSortTime = compareBucketSortTime;
exports.applyCompareSpecToRows = applyCompareSpecToRows;
const dateFormatting_1 = require("./dateFormatting");
const norm = (s) => s.replace(/\s+/g, "").toUpperCase();
function resolveRowColumnKey(row, col) {
    if (!col)
        return null;
...
function shiftBucketLabelOneYear(raw, gran, parseOpts) {
    if (raw == null)
        return null;
    const s = typeof raw === "string" ? raw.trim() : String(raw).trim();
    if (!s && typeof raw !== "string")
        return null;
    if (gran === "quarter") {
        const m = /^T(\d)\s*\/\s*(\d{4})$/i.exec(s.replace(/\s+/g, ""));
        if (m)
            return `T${m[1]}/${Number(m[2]) - 1}`;
    }
    if (gran === "semester") {
        const m = /^S(\d)\s*\/\s*(\d{4})$/i.exec(s.replace(/\s+/g, ""));
        if (m)
            return `S${m[1]}/${Number(m[2]) - 1}`;
    }
    if (gran === "year") {
        const y = Number(s);
    ...
    if (gran === "month") {
        const iso = (0, dateFormatting_1.parseIsoYearMonthForLabel)(s);
        if (iso)
            return `${iso.year - 1}-${String(iso.month).padStart(2, "0")}`;
    }
    const d = (0, dateFormatting_1.parseDateLike)(typeof raw === "string" ? raw : s, parseOpts);
    if (!d)
        return null;
    const y2 = d.getUTCFullYear() - 1;
    const d2 = new Date(Date.UTC(y2, d.getUTCMonth(), d.getUTCDate()));
    return (0, dateFormatting_1.formatDateByGranularity)(d2, gran === "day" || gran === "week" ? gran : "month", undefined, parseOpts);
}
function bucketStartUtc(value, gran, parseOpts) {
    const t = compareBucketSortTime(value, gran, parseOpts);
    if (Number.isNaN(t))
        return null;
    return new Date(t);
}
function shiftCalendarBucketLabel(raw, gran, mode, parseOpts) {
    ...
    if (mode === "calendar_prev_day") {
        ...
        return (0, dateFormatting_1.formatDateByGranularity)(d2, gran === "month" ? "day" : gran, undefined, parseOpts);
    }
    if (mode === "calendar_prev_week") {
        const d2 = new Date(Date.UTC(y, m, day - 7));
        return (0, dateFormatting_1.formatDateByGranularity)(d2, gran, undefined, parseOpts);
    }
    if (gran === "month") {
...
function compareBucketSortTime(value, gran, parseOpts) {
    if (value == null)
        return NaN;
    ...
    const d = (0, dateFormatting_1.parseDateLike)(value, parseOpts);
    return d ? d.getTime() : NaN;
}
function partitionKey(row, dimCols, exclude) {
...
function attachTemporalLag(rows, metricAliases, timeColumn, granularity, dimensionColumns, mode, parseOpts) {
    ...
    if (mode === "prev_bucket") {
        ...
        for (const groupRows of groups.values()) {
            const sorted = [...groupRows].sort((a, b) => compareBucketSortTime(getRowValue(a, timeResolved), granularity, parseOpts) -
...
function attachTemporalCalendarLookup(rows, metricAliases, timeColumn, granularity, dimensionColumns, mode, parseOpts) {
    const timeResolved = timeColumn;
    const mapPrev = new Map();
    for (const row of rows) {
        const tVal = getRowValue(row, timeResolved);
...
function attachFixed(rows, metricAliases, fixed) {
    return rows.map((row) => {
        const next = { ...row };
        for (const alias of metricAliases) {
            const k = resolveRowColumnKey(next, alias);
...
function attachColumnCompare(rows, metricAliases, refColumn) {
    return rows.map((row) => {
        const next = { ...row };
...
function attachAverage(rows, metricAliases, scope, partitionDimensions) {
    const avgs = new Map();
    const groupKey = (row) => scope === "global" ? "__all__" : partitionKey(row, partitionDimensions, undefined);
    for (const row of rows) {
...
function attachTotalShare(rows, metricAliases, partitionDimensions) {
    const sums = new Map();
    for (const row of rows) {
    ...
    return rows.map((row) => {
        ...
        for (const alias of metricAliases) {
            ...
            if (total != null)
                next[`${k}_total_ref`] = total;
        }
        return next;
    });
}
function extractYearMonthFromBucket(raw, gran, parseOpts) {
    const d = (0, dateFormatting_1.parseDateLike)(raw, parseOpts);
    if (d)
        return { year: d.getUTCFullYear(), month1: d.getUTCMonth() + 1 };
    const iso = (0, dateFormatting_1.parseIsoYearMonthForLabel)(raw);
    if (iso)
        return { year: iso.year, month1: iso.month };
    if (gran === "quarter" && typeof raw === "string") {
        const m = /^T(\d)\s*\/\s*(\d{4})$/i.exec(raw.trim().replace(/\s+/g, ""));
        if (m)
            return { year: Number(m[2]), month1: (Number(m[1]) - 1) * 3 + 1 };
    }
    if (gran === "year" && typeof raw === "string") {
        const y = Number(raw.trim());
        if (Number.isFinite(y))
            return { year: y, month1: 1 };
    }
    return null;
}
function attachCumulativeCompare(rows, metricAliases, timeColumn, granularity, dimensionColumns, mode, parseOpts) {
    const timeResolved = timeColumn;
    const yearPartition = (row) => {
    ...
    const ytdByRowMetric = new WeakMap();
    const ytdIndex = new Map();
    for (const groupRows of groups.values()) {
        const sorted = [...groupRows].sort((a, b) => compareBucketSortTime(getRowValue(a, timeResolved), granularity, parseOpts) -
            compareBucketSortTime(getRowValue(b, timeResolved), granularity, parseOpts));
        const runByAlias = {};
...
function applyCompareSpecToRows(rows, metricAliases, spec, opts) {
    if (!rows.length || spec.kind === "none")
        return rows.map((r) => ({ ...r }));
    ...
    if (spec.kind === "cumulative") {
        return attachCumulativeCompare(rows, metricAliases, spec.timeColumn, spec.granularity, dimCols, spec.mode, opts.parseDateOpts);
    }
    if (spec.kind === "temporal") {
        if (spec.mode === "prev_bucket" || spec.mode === "same_period_prior_year") {
            return attachTemporalLag(rows, metricAliases, spec.timeColumn, spec.granularity, dimCols, spec.mode, opts.parseDateOpts);
        }
        if (spec.mode === "calendar_prev_day" ||
            spec.mode === "calendar_prev_week" ||
            spec.mode === "calendar_prev_month" ||
            spec.mode === "calendar_prev_year") {
            return attachTemporalCalendarLookup(rows, metricAliases, spec.timeColumn, spec.granularity, dimCols, spec.mode, opts.parseDateOpts);
        }
    }
    return rows.map((r) => ({ ...r }));
}
//# sourceMappingURL=compareMetricRows.js.map