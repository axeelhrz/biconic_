...
exports.resolveJoinQueryOrigins = resolveJoinQueryOrigins;
exports.callJoinQueryForEtl = callJoinQueryForEtl;
const JOIN_FETCH_TIMEOUT_MS = Number(process.env.ETL_JOIN_TIMEOUT_MS) > 0
    ? Number(process.env.ETL_JOIN_TIMEOUT_MS) + 60_000
    : 650_000;
function joinFetchSignal() {
...
function isBackendApiUrl(url) {
    ...
    return candidates.some((c) => normalized === c || normalized === c.replace(/\/v1$/, ""));
}
function isNextJsRuntime() {
    return !!(process.env.NEXT_RUNTIME || process.env.VERCEL);
}
function isModuleLoadError(message) {
    return /Cannot find module|next\/server|ERR_MODULE_NOT_FOUND|MODULE_NOT_FOUND|No se pudo cargar join-query/i.test(message);
}
...
function resolveJoinQueryOrigins(ctx) {
    const origins = [];
    const add = (raw) => {
        const url = raw?.trim().replace(/\/$/, "");
        if (!url || isBackendApiUrl(url))
            return;
    ...
    if (vercel)
        add(vercel.startsWith("http") ? vercel : `https://${vercel}`);
...
async function parseJoinResponse(res) {
...
async function tryJoinQueryInProcess(body, errors) {
    try {
...
async function tryJoinQueryHttp(body, ctx, errors) {
    const headers = {
...
async function callJoinQueryForEtl(joinQueryBody, ctx) {
    const body = { ...joinQueryBody, fromEtlRun: true };
    const errors = [];
    const strategies = isNextJsRuntime()
        ? [
    ...
    const prodHint = " En Railway, configurá NEXT_INTERNAL_URL con la URL de Vercel.";
    throw new Error(`Error ejecutando JOIN múltiple: ${errors[errors.length - 1] || "sin respuesta"}.${localHint}${prodHint} Intentos: ${errors.slice(0, 5).join(" | ")}`);
}
//# sourceMappingURL=call-join-query-for-etl.js.map