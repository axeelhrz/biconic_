...
function jsonResponse(data: unknown, init?: { status?: number }): AggregateDataResult {
...
async function fetchTableColumnNames(
  ...
  const resolvedUrl = dbUrl ?? getInternalDbUrl();
...
export async function runAggregateData(
  body: AggregationRequest,
  deps: AggregateDataDeps
