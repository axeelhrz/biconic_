...
export default function NewClientPage() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    try {
      ...
      if (!res.ok) {
        const errorMsg = res.error ?? "No se pudo crear el cliente";
        const errorDetails = (res as { details?: string })?.details;
      ...
      <FormProvider {...methods}>
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm" style={{ color: "var(--platform-fg-muted)" }}>
            Creá cliente empresa o cliente individuo y asigná el usuario administrador.
          </p>
        </div>
